<img width="3780" height="866" alt="512435110-51412d5c-19ce-4104-ad56-98808a0bd5ef" src="https://github.com/user-attachments/assets/bcc3e675-3b22-425e-b57b-35ad004195c4" />

# Olá, mundo! 👋 Meu nome é Carlos Augusto Nunes Vieira

## Desenvolvedor Full Stack (em formação) | Entusiasta de Tecnologia | Estudante de Engenharia de Software

Sou um apaixonado por tecnologia e por criar soluções eficientes e escaláveis. Atualmente, estou cursando **Engenharia de Software** na **Universidade Federal do Amazonas (UFAM)** e mergulhando de cabeça no universo do desenvolvimento web.

### Instituto de Computação - UFAM
![512433746-9ec41c54-fd65-4047-a33a-0497cfdff54a](https://github.com/user-attachments/assets/006a079b-2911-4b39-9525-6198c6b8b378)

No momento, aplico meus conhecimentos no **Instituto de Desenvolvimento Tecnológico (INDT)**, onde contribuo para a modernização e otimização dos processos internos.

### Instituto de Desenvolvimento Tecnológico
![indt](https://github.com/user-attachments/assets/141bcad1-07c7-41e0-a3ae-fdd40aa6f148)
---

## 🚀 Minha Jornada de Aprendizado

Acredito que o aprendizado contínuo é a chave! Estou sempre buscando evoluir e dominar novas ferramentas.

* 🎓 **Graduação:** Engenharia de Software (Universidade Federal do Amazonas - UFAM)
* 🏁 **Concluindo:** Curso "Do Zero ao Profissional" Full Stack Developer (Sujeito Programador)
* 🔥 **Novo Desafio:** Dei início ao **FullStack Pro** (Sujeito Programador), um programa completo focado em me levar ao próximo nível com as tecnologias mais utilizadas no mercado, do front-end ao back-end!

---

## 💡 O que eu faço

Meu foco é transformar problemas complexos em soluções simples e funcionais.

* 💻 Desenvolvimento de Software
* 🤔 Análise e Solução de Problemas (adoro um bom desafio!)
* 🤝 Colaboração em Equipes de TI
* 📚 Aprendizado Contínuo de Novas Tecnologias

---

## 🛠️ Meu Kit de Ferramentas (Principais Tecnologias)

Esta é a base que estou solidificando e expandindo todos os dias:

* **HTML5**
* **CSS3**
* **JavaScript**
* **React JS**
* **Java**
* **Liguagem C**

*(...e muitas outras sendo adicionadas ao arsenal com o FullStack Pro!)*

---

## 🎯 Meu Objetivo

Estou em busca de **oportunidades desafiadoras** (como estágios, vagas júnior ou projetos freelancer) para aprimorar minhas habilidades, especialmente em **desenvolvimento full stack**, e colaborar em projetos que gerem um impacto positivo.

Que venham os desafios, os códigos e as conquistas! 💻

---

## 📫 Vamos nos conectar?

Adoraria trocar uma ideia sobre tecnologia, projetos ou oportunidades!

* **LinkedIn:** www.linkedin.com/in/carlos-augusto-nunes-vieira-602833304
* **Email:** augustovieira@icomp.ufam.edu.br
